...
const vite = await createServer({
  ...
  plugins: [{ name: "monthly-test-env", resolveId(id) { if (id === "@/lib/runtime-env" || id.replaceAll("\\", "/").match(/\/lib\/runtime-env(?:\.ts)?$/)) return "\0monthly-env"; }, load(id) { if (id === "\0monthly-env" || id.replaceAll("\\","/").endsWith("/lib/runtime-env.ts")) return "export const env = globalThis.__MONTHLY_TEST_ENV__"; } }],
