...
export default function LegalPage() {
  ...
  return <main className="legal-page"><Link href="/">← Radiant Reviewに戻る</Link><h1>販売条件・運営情報</h1>
    ...
    <h2>特定商取引法に基づく表記</h2><dl>
      ...
      <dt>販売価格</dt><dd>Review：900円（税込）、Climb：1,800円（税込）。各30日・月の立ち回り枠はRiot AI 50試合、Replay Coach 5試合、Deep Coach 2試合です。AIM枠はReview 5試合、Climb 10試合です。各試合につき選択した最大3場面を解析します。</dd>
      ...
      <dt>利用範囲・上限</dt><dd>60分以内の対応録画が対象です。Replay Coachはデス前20秒〜後5秒、AIMは選択時刻の前後0.4秒、Deep Coachは指定した10秒〜5分の区間から最大6枚を解析します。Riot AIはRiot公式承認後、Riotアカウント連携とWindows版Radiant AIアプリが必要です。1アカウント1日12回（失敗・保留を含む、日本時間）まで受け付けます。全体の受付上限や混雑により翌日以降の受付となる場合があります。未使用枠の繰越はありません。</dd>
      <dt>解析枠</dt><dd>有料期間ではRiot AI、Replay Coach、Deep Coach、AIMを別々に数えます。各モードは最初の解析成功で1試合分を消費し、同じ録画の残り2場面を同じ契約期間内に解析できます。無料体験は全モード合計1試合です。解析失敗・判定保留では試合枠を消費しません。同じ場面の再表示は保存済み結果を返し、追加消費しません。</dd>
      ...
      <dt>動作環境</dt><dd>録画ファイルを扱えるPCブラウザとインターネット接続が必要です。Riot AIのみWindowsにインストールしたRadiant AIアプリが必要です。MP4・WebM・MOVはブラウザが再生できる形式に限ります。購入前に無料の録画切り出し機能で再生を確認してください。</dd>
