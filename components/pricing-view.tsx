...
type Plan = "card_monthly" | "paypay_30day";
...
const TIERS: ReadonlyArray<{ name: string; price: number; card: Plan; paypay: Plan }> = [{ name: "Radiant AI", price: 900, card: "card_monthly", paypay: "paypay_30day" }];
...
export function PricingView() {
  ...
  return <div className="portal-page pricing-page"><SiteHeader current="pricing" /><main className="portal-main"><PageLead eyebrow="PRICING" title="月額900円、3つの解析枠" description="立ち回りは最大50試合、ミクロは最大5試合、Deepは最大2試合。立ち回りのリプレイ選択とAI自動判断は同じ枠を使います。未使用分は月2試合まで別モードへ振替できます。" />
    ...
    <section className="pricing-grid single">{TIERS.map(tier => <article key={tier.name} className="featured"><span className="pricing-recommend">全モード込み</span><small>MONTHLY PLAN</small><h2>{tier.name}</h2><div className="pricing-value"><strong>¥{tier.price.toLocaleString()}</strong><span>税込 / 30日・月</span></div><ul><li><CheckCircle2 /> 立ち回り：最大50試合（2つの方法で共通）</li><li><CheckCircle2 /> ミクロ：最大5試合</li><li><CheckCircle2 /> Deep：最大2試合</li><li><CheckCircle2 /> 未使用分を月2試合まで振替</li><li><CheckCircle2 /> AI解析費用込み</li><li><CheckCircle2 /> 成長グラフ・30日ミッション</li></ul><div className="pricing-actions"><Button disabled={busy !== null || Boolean(status?.entitlement) || Boolean(status && !status.configured)} onClick={() => void checkout(tier.card)}>{busy === tier.card ? <LoaderCircle className="spin" /> : <CreditCard />}{account.snapshot?.user ? "カードで始める" : "ログインして選ぶ"}</Button><Button variant="outline" disabled={busy !== null || Boolean(status?.entitlement) || !status?.configured || !status.paypayEnabled} onClick={() => void checkout(tier.paypay)}><QrCode /> PayPay 30日パス</Button></div></article>)}</section>
    ...
    <section className="pricing-rules"><ShieldCheck /><div><h2>解析枠の数え方</h2><p>立ち回り・ミクロ・Deepを別々に数えます。未使用枠がある場合、合計2試合まで別モードへ自動振替できます。保存済みの同じ結果を再表示しても枠は減りません。無料体験は全モード合計1試合です。</p><p>立ち回りとミクロはWindows版Radiant AIアプリから自動同期します。PayPayは30日間の一回払い、カードは月額自動更新です。</p></div></section>
  </main></div>;
}
