...
export function PricingView() {
  ...
  return <div className="portal-page pricing-page"><SiteHeader current="pricing" /><main className="portal-main"><PageLead eyebrow="PRICING" title="料金はそのまま、立ち回り枠を拡張" description="立ち回りはRiot AI 50試合・Replay Coach 5試合・Deep Coach 2試合。AIMは現在のプラン枠で利用できます。" />
    ...
    <section className="pricing-rules"><ShieldCheck /><div><h2>解析枠の数え方</h2><p>有料期間では、立ち回り3種類とAIMを別々に数えます。同じモード・同じ録画は最大3解析で、保存済みの同じ結果を再表示しても枠は減りません。無料体験は全モード合計1試合です。</p><p>Riot AIはRiot公式承認後、Riotアカウントを連携し、WindowsにインストールしたRadiant AIアプリから利用できます。PayPayは30日間の一回払い、カードは月額自動更新です。</p></div></section>
  </main></div>;
}
