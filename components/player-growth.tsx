...
export function PlayerGrowth({ refreshKey = 0 }: { refreshKey?: number }) {
  ...
  return <section className="player-growth panel" id="player-growth" aria-labelledby="player-growth-title">
    ...
    <details className="pg-guide"><summary><BookOpen /> 評価基準と記録の使い方</summary><div><p>自己評価は、今回練習・確認できた項目だけを選びます。未評価は0点ではありません。4・5は別の場面や試合での再現を確認してから選んでください。</p><ol>{LEVEL_LABELS.slice(1).map(text => <li key={text}>{text}</li>)}</ol><p>AIレビューは少数の静止画で確認できる行動を1〜3で評価します。AIMモードでは同じ敵と照準が見える3枚以上を根拠にAIMを評価し、照準の置き方も確認します。立ち回りモードでは位置取り・ピーク判断などを確認します。自己評価とAI評価は基準・観測範囲が異なるため混ぜません。表示対象は種類ごとに最新120件。自己評価は1日20件までです。サンプル・判定保留・評価できない項目は集計に含みません。</p></div></details>
