  ...
  return <section className="player-growth panel" id="player-growth" aria-labelledby="player-growth-title">
    ...
    <div className="pg-main">
      ...
      <aside className="pg-detail" aria-live="polite">
        ...
        {latestEvidence && source === "ai" ? <div className="pg-evidence"><h4>直近のAI評価の根拠</h4><p>{latestEvidence.evidence}</p><small>録画時刻 {latestEvidence.times.map(t => `${Math.floor(t / 60)}:${String(Math.floor(t % 60)).padStart(2, "0")}`).join(" / ")}</small></div> : null}
    ...
    <div className="pg-skill-list" aria-label="項目別の成長と練習メニュー">{comparison.axes.map((axis, i) => {
      ...
      return <button type="button" key={axis.id} className={selected === axis.id ? "selected" : ""} aria-pressed={selected === axis.id} onClick={() => setSelected(axis.id)}><span className="pg-skill-number">{String(i + 1).padStart(2, "0")}</span><span className="pg-skill-name"><strong>{axis.label}</strong><small>{axis.current.count ? `${axis.current.count}件 · 前期間 ${axis.previous.value ?? "—"}点` : source === "ai" && !axis.ai ? "自己評価で確認" : "未評価"}</small></span><span className="pg-skill-value">{axis.current.value ?? "—"}<small className={change !== null && change > 0 ? "positive" : ""}>{change === null ? "" : change > 0 ? `+${change}` : change}</small></span></button>;
    })}</div>
    <details className="pg-guide"><summary><BookOpen /> 評価基準と記録の使い方</summary><div><p>自己評価は、今回練習・確認できた項目だけを選びます。未評価は0点ではありません。4・5は別の場面や試合での再現を確認してから選んでください。</p><ol>{LEVEL_LABELS.slice(1).map(text => <li key={text}>{text}</li>)}</ol><p>AIレビューは少数の静止画で確認できる行動を1〜3で評価します。自己評価とAI評価は基準・観測範囲が異なるため混ぜません。表示対象は種類ごとに最新120件。自己評価は1日20件までです。サンプル・判定保留・評価できない項目は集計に含みません。</p></div></details>
