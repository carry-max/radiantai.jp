...
export function JevDeveloperView({ userId, email, serverConfigured }: { userId: string; email: string; serverConfigured: boolean }) {
  ...
  return <main className="jev-console">
    ...
    <section className="jev-hero"><div><p>RADIANT AI / INTERNAL</p><h1>AI Match Classifier</h1><span>Riot API・Replayから整形した試合データを、最大50件まで高速分類します。</span></div><div className="jev-model"><BrainCircuit /><span><small>MODEL</small><strong>AI</strong></span></div></section>
    <section className="jev-flow" aria-label="解析フロー"><span>Riot API / Replay</span><i>→</i><span>AI</span><i>→</i><span>50試合を分類</span><i>→</i><span>Deep解析候補</span></section>
    <section className="jev-grid">
      <div className="jev-panel"><div className="jev-panel-title"><KeyRound /><div><h2>認証</h2><p>{email} のみ表示中</p></div></div><label>AI Gateway APIキー<input type="password" autoComplete="off" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder={serverConfigured ? "サーバー設定済み（空欄で使用）" : "AI_GATEWAY_API_KEY"} /></label><small><ShieldCheck /> 入力値は保存せず、このリクエストだけに使用します。</small></div>
      <div className="jev-panel jev-data"><div className="jev-panel-title"><BrainCircuit /><div><h2>試合データ</h2><p>{count} / 50 試合</p></div></div><textarea spellCheck={false} value={source} onChange={event => setSource(event.target.value)} /><Button onClick={() => void run()} disabled={busy || count < 1 || count > 50}>{busy ? <LoaderCircle className="spin" /> : <Play />}{busy ? "分類しています…" : "AIで分類"}</Button></div>
    </section>
    {result?.error ? <p className="jev-error" role="alert">{result.error}</p> : null}
    {result?.summary ? <section className="jev-results">
      <div className="jev-result-head"><div><p>CLASSIFICATION RESULT</p><h2>{result.summary.total}試合の分類完了</h2></div><span><CheckCircle2 /> AI</span></div>
